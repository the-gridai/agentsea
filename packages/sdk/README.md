...
- **No control-plane HTTP API** — AgentSea has no server-side control plane: it's a static manifest plus your cloud accounts plus local history.
- **No provisioning logic** — provisioning, auth, and cloud orchestration all live in the `agentsea` CLI, not the SDK.
