...
export function validateIdentifier(identifier: string, fieldName: string): void {
  if (!identifier || identifier.trim() === "") {
    const listCmd = fieldName.toLowerCase().includes("agent") ? `${AGENTSEA_CLI} agents` : `${AGENTSEA_CLI} clouds`;
    throw new Error(
  ...
  if (!script || script.trim() === "") {
    throw new Error(
      "The downloaded script is empty.\n\n" +
        ...
        `  2. Verify the combination exists: ${AGENTSEA_CLI} matrix\n` +
  ...
  if (!filePath || filePath.trim() === "") {
    throw new Error(
      `Prompt file path is required when using --prompt-file.\n\n` +
        `Example:\n` +
        `  ${AGENTSEA_CLI} <agent> <cloud> --prompt-file instructions.txt`,
    );
  }
    ...
    if (pattern.test(resolved)) {
      throw new Error(
        `Security check failed: cannot use '${filePath}' as a prompt file.\n\n` +
          ...
          `  2. Use it: ${AGENTSEA_CLI} <agent> <cloud> --prompt-file prompt.txt`,
      );
    }
  }
  ...
  if (!prompt || prompt.trim() === "") {
    throw new Error(
      "Prompt is required but was not provided.\n\n" +
        ...
        `  ${AGENTSEA_CLI} <agent> <cloud> --prompt "Your task here"\n\n` +
        "Or use a file:\n" +
        `  ${AGENTSEA_CLI} <agent> <cloud> --prompt-file prompt.txt`,
    );
  }
  ...
  if (prompt.length > MAX_PROMPT_LENGTH) {
    ...
    throw new Error(
      `Your prompt is too long (${lengthKB}KB, maximum is 10KB).\n\n` +
        ...
        `  2. Use the file: ${AGENTSEA_CLI} <agent> <cloud> --prompt-file instructions.txt\n\n` +
