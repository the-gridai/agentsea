...
import { isString, toRecord } from "@agentsea/sdk";
...
export async function cmdStatus(opts: StatusOpts = {}): Promise<void> {
  ...
  if (candidates.length === 0) {
    ...
    p.log.info(`Run ${pc.cyan(`${AGENTSEA_CLI} <agent> <cloud>`)} to launch your first agent.`);
    return;
  }
  ...
  if (opts.prune && goneRecords.length > 0) {
    ...
    p.log.info(
      pc.dim(
        `${goneRecords.length} server${goneRecords.length !== 1 ? "s" : ""} marked as gone. Run ${pc.cyan(`${AGENTSEA_CLI} status --prune`)} to remove them.`,
      ),
    );
  }
  ...
  if (running > 0) {
    p.log.info(
      pc.dim(`${running} server${running !== 1 ? "s" : ""} running. Use ${pc.cyan(`${AGENTSEA_CLI} list`)} to reconnect.`),
    );
  }
}
