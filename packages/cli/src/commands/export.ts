// commands/export.ts — `agentsea export [name|id]`
//
// Captures a running claude agentsea as a redistributable github repo. The
// output is the symmetric inverse of `--repo`: today `agentsea claude hetzner
// --repo user/template` consumes a repo. After `agentsea export`, the user
// gets a `agentsea claude <cloud> --repo user/<slug>` line they can hand off
// or re-run.
//
// v1 scope: claude only.
// - When the user has multiple claude agentseas, a picker lists them.
...
const REMOTE_RESULT_PATH = "/tmp/agentsea-export-result.json";
...
/** Resolve the --steps value to bake into the agentsea link. */
export function resolveSteps(record: AgentseaRecord): string {
...
function exportableClaudeRecords(records: AgentseaRecord[]): AgentseaRecord[] {
...
/** Find a claude agentsea by name or id. */
function matchTarget(records: AgentseaRecord[], target: string): AgentseaRecord | null {
  return records.find((r) => r.id === target || r.name === target || r.connection?.server_name === target) ?? null;
}

/** Build the agentsea.md content from a record. Re-spawning consumes this. */
export function buildAgentseaMd(record: AgentseaRecord): string {
  const lines: string[] = [
    "---",
  ];
  if (record.name) {
  ...
  lines.push(`description: ${JSON.stringify(`Exported from agentsea ${record.id}`)}`);
  lines.push("---");
  lines.push("");
  lines.push(`# ${record.name ?? `${AGENTSEA_CLI} export`}`);
  lines.push("");
  lines.push(`This template was generated by \`${AGENTSEA_CLI} export\`. Re-agentsea it with the`);
...
export function buildReadmeTemplate(): string {
  return [
    ...
    "",
    "```bash",
    `${AGENTSEA_CLI} claude __CLOUD__ --repo __SLUG__ --steps __STEPS__`,
    "```",
    "",
    "Re-spawning is non-interactive — the `--steps` list bakes in the same",
    "setup decisions the original agentsea made, so you won't be prompted.",
    "",
    "## First-run checklist",
    "",
    ...
    "- `agentsea.md` — machine-readable re-agentsea metadata",
    "",
  ].join("\n");
}
...
export function buildExportScript(opts: {
  ...
  return [
    ...
    "",
    "# 1. Heredoc the static files (agentsea.md, .gitignore, README template)",
    `cat > "$EXPORT_DIR/agentsea.md" <<'AGENTSEA_MD_EOF'`,
    opts.agentseaMd,
    "AGENTSEA_MD_EOF",
    "",
    ...
    "fi",
    'if [ -z "$PROJECT_NAME" ]; then',
    '  PROJECT_NAME="agentsea-export-$(date +%s)"',
    "fi",
    "",
    ...
    "",
    "# 10. Commit and push.",
    'git -c user.email=agentsea-export@thegrid.ai -c user.name="agentsea export" commit -q -m "agentsea export"',
    "",
    'gh repo create "$SLUG" "$VISIBILITY_FLAG" --source=. --push --description="Exported with agentsea"',
    "",
...
/** Build the runner for a specific agentsea record. Sprite has its own exec
 *  channel (`sprite exec`, etc.); everything else uses SSH. */
async function buildRunnerForRecord(record: AgentseaRecord): Promise<ExportRunner> {
  const conn = record.connection;
  if (!conn) {
    throw new Error("Cannot build runner: agentsea has no connection info.");
  }
...
/** Pick one record from a list of claude agentseas. */
async function pickOne(records: AgentseaRecord[]): Promise<AgentseaRecord | null> {
  ...
  const choice = await p.select({
    message: "Which claude agentsea do you want to export?",
    options,
  });
...
export interface ExportOptions {
  ...
  records?: AgentseaRecord[];
}

/** Top-level command: `agentsea export [target]`. */
export async function cmdExport(target: string | undefined, options?: ExportOptions): Promise<void> {
  ...
  if (exportable.length === 0) {
    p.log.info("No claude agentseas available to export.");
    p.log.info(`Run ${pc.cyan(`${AGENTSEA_CLI} claude <cloud>`)} first, then export the result.`);
    process.exit(1);
  }

  let picked: AgentseaRecord | null = null;
  if (target) {
  ...
  console.log();
  console.log(pc.dim("Re-agentsea with:"));
  console.log(`  ${pc.cyan(`${AGENTSEA_CLI} ${CLAUDE_AGENT} ${r.cloud} --repo ${parsed.slug} --steps ${steps}`)}`);
  console.log();
}
...
async function runPassAndParseResult(
  ...
  const localTmp = mkdtempSync(join(tmpdir(), "agentsea-export-"));
